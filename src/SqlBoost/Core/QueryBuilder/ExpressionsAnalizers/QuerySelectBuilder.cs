	...
	public class QuerySelectBuilder : QueryExpressionBuilder
	{
		...
		protected override Expression VisitMemberInit(MemberInitExpression node)
		{
			if (node.NewExpression.Constructor.GetParameters().Length > 0)
				throw new SqlBoostException("You can use either Object Initializer or Constructor with parameters to create entity instance");
		...
		protected override Expression VisitNew(NewExpression node)
		{
			...
			for (int i = 0; i < node.Arguments.Count; i++)
			{
				if (node.Arguments[i].NodeType == ExpressionType.Parameter)
					throw new SqlBoostException("In anonimus type you should provide only single fields, not whole classes");
