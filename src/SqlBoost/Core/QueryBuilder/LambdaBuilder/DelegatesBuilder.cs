...
{
	internal abstract class DelegatesBuilder : IDelegatesBuilder
	{
			...
			if (expQueue.Count > 1)
			{
				while (expQueue.Count != 0)
				{
					...
					if (exp == null) // static type
						throw new SqlBoostException("Invalid constant accessor detected. You can not use static fields as constants holders. Consider passing this value as variable");
					...
					else throw new SqlBoostException("Invalid constant accessor detected");
				}
			}
			else if (expQueue.Peek().NodeType != ExpressionType.Constant)
				throw new SqlBoostException("Invalid constant accessor detected");
