...
namespace SqlBoost.Tests.IntegrationTests
{
	...
	public class SelectTests : TestBase
	{
		...
		{
			var c = "cost";
			var scalar = EfQuery.Select((db) => c).ExecuteScalar();
		...
		public void Select_Constants_In_AnonimusType_Integration()
		{
			var c = "cost";
			var i = 43;
			var res = EfQuery.Select((db) => new { c, i }).ExecuteQuery().ToArray();
		...
		public void Select_Constants_In_DtoType_InitByConstructor_Integration()
		{
			var c = "cost";
			var i = 43;
			var res = EfQuery.Select((db) => new Dto1(c, i)).ExecuteQuery().ToArray();
			Assert.AreEqual(1, res.Length);
		...
		public void Select_Constants_In_DtoType_InitByParams_Integration()
		{
			var c = "cost";
			var i = 43;
			var res = EfQuery.Select((db) => new Dto1() { Field1 = c, Field2 = i }).ExecuteQuery().ToArray();
		...
		{
			var res = EfQuery.From<Product>().Select((db, p) => p).ExecuteQuery().ToArray();
		...
		public void Select_OneField_Integration()
		{
			var res = EfQuery.From<Product>()
							.Select((db, p) => p.ProductName).ExecuteQuery().ToArray();
			Assert.AreEqual(77, res.Length);
		...
		{
			var c = "constant";
			var res = EfQuery.From<Product>().Select((db, p) => c).ExecuteQuery().ToArray();
			Assert.AreEqual(77, res.Length);
		...
		{
			var res = EfQuery.From<Product>()
			.Select((db, p) => new { Fld1 = p.CategoryID, Fld2 = p.Discontinued }).ExecuteQuery().ToArray();
		...
		{
			var res = EfQuery.From<Product>().Select((db, p) => db.Substring(p.ProductName, 1, 5)).ExecuteQuery().ToArray();
			Assert.AreEqual(77, res.Length);
		...
		{
			var res = EfQuery.From<Product>().Select((db, p) => db.Count(p.ProductID)).ExecuteScalar();
		...
		{
			var c = "cost";
			var res = EfQuery.Select(db => db.Substring(c, 1, 3)).ExecuteScalar();
		...
		{
			var c = "cost";
			var res = EfQuery.Select(db => new { res = db.Substring(c, 1, 3), c }).ExecuteQuery().ToArray();
		...
		public void Select_PassEntity_ButSelectConstant()
		{
			var c = "constant";
			var res = EfQuery.From<Product>()
				.Select((db, p) => db.Substring(c, 1, 4)).ExecuteQuery().ToArray();
			Assert.AreEqual(77, res.Length);
		...
		public void Select_GroupBy()
		{
			var res = EfQuery.From<Product>()
				...
				.Where((db,p)=> p.CategoryID>2 && db.Avg(p.ReorderLevel)>15)
				.Select((db, p) => new { Fld1 = p.CategoryID, Fld2 = db.Avg(p.ReorderLevel)});
		...
		public void Select_CompiledQuery()
		{
			...
			var q = mng.CompileQuery(
						...
						.GroupBy((db, p) => new { p.CategoryID })
						.Where((db, p) => p.CategoryID > args.Item1 && db.Avg(p.ReorderLevel) > 15)
						.Select((db, p) => new { Fld1 = p.CategoryID, Fld2 = db.Avg(p.ReorderLevel) }));
